...
% Original Author: Ahmed Alkhateeb
% Date: Sept. 5, 2018
%
% Modified by Haoran Sun to suit for continual learning
% Date: Feb. 2, 2020
%
...
function DL_CoordinatedBeamforming(input_a, input_b)

% ------  DeepMIMO Dataset Generation  ------------------------------- %
[DeepMIMO_dataset,params]=DeepMIMO_Dataset_Generator(input_a, input_b);
...
for u=1:1:params.num_user
    for t=1:length(params.active_BS)
        Ch=DeepMIMO_dataset{t}.user{u}.channel;
...
sfile_name=strcat('DLCB_Dataset/DLCB_input',int2str(input_a),'_',int2str(input_b),'.mat');
save(sfile_name, 'DL_input')
sfile_name=strcat('DLCB_Dataset/DLCB_output',int2str(input_a),'_',int2str(input_b),'.mat');
save(sfile_name, 'DL_output')
